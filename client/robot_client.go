...
func New(accessToken string) *RobotClient {
	return NewClient(accessToken, dingtalkUrl)
}

func NewClient(accessToken string, baseAPI string) *RobotClient {
	if baseAPI == "" {
		baseAPI = dingtalkUrl
	}
	return &RobotClient{accessToken: accessToken, apiUrl: baseAPI}
}

func (w *RobotClient) sendPayload(payload *PayLoad) error {
...
func (w *RobotClient) SendTextMsg(content string, isAtAll bool, mobiles ...string) error {
	//  send request
...
func (w *RobotClient) SendLinkMsg(title, content, picURL, msgURL string) error {
	return w.sendPayload(&PayLoad{
		MsgType: "link",
		Link: struct {
			Title      string `json:"title"`
...
func (w *RobotClient) SendMarkdownMsg(title, content string, isAtAll bool, mobiles ...string) error {
...
func (w *RobotClient) SendActionCardMsg(title, content string, linkTitles, linkUrls []string, hideAvatar, btnOrientation bool) error {
	//  validation is empty
...
func (w *RobotClient) SendLinkCardMsg(messages []LinkMsg) error {
	return w.sendPayload(&PayLoad{
		MsgType: "feedCard",
		FeedCard: struct {
			Links []LinkMsg `json:"links"`
...
// getSign get sign
func (w *RobotClient) getSign() (timestamp, sha string) {
